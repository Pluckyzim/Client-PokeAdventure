...
function init()
  ...
  autolootbtn = modules.client_topmenu.addRightGameToggleButton('autoloot', tr('Auto-Loot'), '/images/topbuttons/autoloot_hover', exibir)
...
ProtocolGame.registerExtendedOpcode(109, function(protocol, opcode, buffer) -- receive player items added
  ...
  local btn = g_ui.createWidget("UIButton", panel_items)
  btn:setImageSource("images/button")
  btn:setImageBorder(15)
  btn.onHoverChange = function(self, hovered)
    if hovered then
      btn:setImageSource("images/button_hover")
    else
      btn:setImageSource("images/button")
    end
  end
